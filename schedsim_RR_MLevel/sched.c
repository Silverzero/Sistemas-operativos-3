...
sched_class_t* active_sched_class=&rr_mlev_sched; /* Default RR */
...
static void init_runqueue(runqueue_t* rq, int cpu) {
    ...
    init_slist(&(rq->level_tasks[0]),sizeof(task_t),offsetof(task_t,rq_links));
    init_slist(&(rq->level_tasks[1]),sizeof(task_t),offsetof(task_t,rq_links));
    init_slist(&(rq->level_tasks[2]),sizeof(task_t),offsetof(task_t,rq_links));
